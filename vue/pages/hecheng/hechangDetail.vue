		...
		<view class="content">
			...
			<view class="type1">
				<view class="msgBox">
					...
					<view class="flex_bt">
						<view class="category_name">碎片合成</view>
					</view>
					<!-- <view class="describe">{{info.title}}</view> -->
				</view>
				<view class="descBox" v-if="info.content">
					<view class="item">合成介绍</view>
					<view class="desinfo" v-html="util.checkImg(info.content)"></view>
				</view>
				<view class="footerBox flex_bt">
					<view class="price">剩余合成次数:<text class="no_open">{{number}}</text></view>
					<view class="subBtn " v-if="number!=0" @tap="openManghe()">立即合成</view>
			...
			<uni-popup ref="popup" class="win_popup" type="center" :mask-click="false" :animation="false">
				<view class="popup-content">
					...
					<view class="manghe_content">
						...
						<view class="desc" v-if="winInfo">{{info.name}}</view>
						<view class="no_win">{{winInfo}}!</view>
...
<script>
	export default {
		...
		methods: {
			openManghe() {
				this.is_mask = true;
				this.$http.post('index/chipCompose', {
					id: this.goodsId
				}).then(res => {
					if (res.code == 1) {
						this.getData();
			...
			getData() {
				this.$http.post('index/synthesis', {
					id: this.goodsId
				}).then(res => {
					this.info = res.goodslist;
					this.number = res.hcsycs;
				})
			}
		}
	}
