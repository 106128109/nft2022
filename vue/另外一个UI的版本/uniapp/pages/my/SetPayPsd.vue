<template>
	<view class="content">
		<view class="iptBox flexBox">
			...
			<input v-model="phone" type="number" disabled  maxlength="11" placeholder="请输入手机号码" class="ipt" placeholder-class="iptP" />
		</view>
		...
		methods: {
			...
			sub() {
				...
				const data = {
					...
					pay_password_re: this.newPass,
					type: 2
				};
				this.$http.post('user/updatePayPassword', data).then(res => {
					if (res.code == 1) {
						this.toast(res.msg);
