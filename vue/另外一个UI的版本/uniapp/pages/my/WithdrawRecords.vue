<template>
	<view class="content">
		...
		<view class="item ptb-22 bb flex flex-between" v-for="(item,index) in recordsList" :key="index" v-if="recordsList.length">
			<view>
				<view class="size-32 white mb-10">{{item.type=='1'?'银行卡':item.type=='2'?'支付宝':'微信'}}提现</view>
				<view class="colorc6 size-28">{{item.create_time}}</view>
			</view>
			<view>
				<view class="size-32 white mb-10">-{{item.account}}</view>
				<view class="colorc6 size-28 textend" v-if="item.status == '0'">审核中</view>
				<view class="mcolor1 size-28 textend" v-if="item.status == '1'">已拒绝</view>
