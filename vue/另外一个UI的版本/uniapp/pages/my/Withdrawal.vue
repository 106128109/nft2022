<template>
	<view class="content  container">
		<view class="WithdrawalWarp">
			<view class="WithdrawalBox flex_bt">
				<view class="left">
					...
					<view class="price">
						<block v-if="flag" >¥ <text>{{account}}</text></block>
						<block v-else>****** </block>
					<image @tap="flag=!flag" class="rightIcon" :src=" flag?'../../static/img/nosee.png' :'../../static/img/see.png' " mode=""></image>
					</view>
				</view>
				...
				<radio-group @change="payTypeChange">
					<label class="item" v-for="(pay,index) in payList" :key="index">
						<radio style="transform:scale(0.7)" :value="pay.id.toString()" :checked="payType == pay.id"
							color="#00DB7D" />
						<text>{{pay.name}}</text>
						<image v-if="pay.id == '1'" src="../../static/img/my/yinlian.png" mode=""></image>
		...
		<view class="listItem2 flexBox" style="margin: 0 30rpx">
