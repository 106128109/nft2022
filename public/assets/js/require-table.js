define(['jquery', 'bootstrap', 'moment', 'moment/locale/zh-cn', 'bootstrap-table', 'bootstrap-table-lang', 'bootstrap-table-export', 'bootstrap-table-commonsearch', 'bootstrap-table-template', 'bootstrap-table-jumpto', 'bootstrap-table-fixed-columns'], function ($, undefined, Moment) {
    var Table = {
        ...
        config: {
            ...
            editbtn: '.btn-edit',
            delbtn: '.btn-del',
           //plrechargebtn: '.btn-plrecharge',
           plgivebtn: '.btn-plgive',
        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                //批量充值
                // toolbar.on('click', Table.config.plrechargebtn, function () {
                //     var ids = Table.api.selectedids(table);
                //     var url = options.extend.plrecharge_url;
                //     Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('Plrecharge'), $(this).data() || {});
                // });
                
                 
                //批量赠送
                toolbar.on('click', Table.config.plgivebtn, function () {
                     var ids = Table.api.selectedids(table);
                    //Table.api.multi($(this).data("action"), ids, table, this);
                    var ids = Table.api.selectedids(table);
                    var url = options.extend.plgive_url;
                    Fast.api.open(url, $(this).data("original-title") || $(this).attr("title") || __('Plgive'), $(this).data() || {ids});
                });
                // 拖拽排序
