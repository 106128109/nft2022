...
(function (window, undefined) {
    ...
    function _mediaType(src) {
        ...
        if (/\.(mp4|mpg)(\?|$)/i.test(src)) {
            return 'video/mpeg4';
        }
        if (/\.(mp3|wav|ogg|acc|webm)(\?|$)/i.test(src)) {
            var suffix = src.split('.').pop();
            return 'audio/' + (suffix == 'mp3' ? 'mpeg' : suffix);
        }
    ...
    function _mediaEmbed(attrs) {
        var type = attrs.type == 'video/mpeg4' ? 'video' : (attrs.type == 'audio/mpeg' ? 'audio' : 'embed');
    ...
    KEditor.prototype = {
        ...
        errorDialog: function (html) {
            ...
            var dialog = self.createDialog({
                width: Math.min(document.body.clientWidth, 750),
    ...
    _plugin('core', function (K) {
        ...
        self.beforeSetHtml(function (html) {
            ...
            return html.replace(/<embed[^>]*type="([^"]+)"[^>]*>(?:<\/embed>)?/ig, function (full) {
                ...
                .replace(/<(video|audio)[^>]*type="([^"]+)"[^>]*>(?:<\/(video|audio)>)?/ig, function (full) {
                    var attrs = _getAttrList(full);
                    attrs.src = _undef(attrs.src, '');
                    attrs.width = _undef(attrs.width, 0);
                    attrs.height = _undef(attrs.height, 0);
...
KindEditor.lang({
    ...
    invalidImg: "请输入有效的URL地址。\n只允许jpg,gif,bmp,png,webp格式。",
    invalidMedia: "请输入有效的URL地址。\n只允许swf,flv,mp3,wav,wma,wmv,mid,avi,mpg,asf,rm,rmvb,webm格式。",
...
KindEditor.plugin('graft', function (K) {
    ...
    self.plugin.graftDialog = function (options) {
        ...
        var html = [
            ...
            '<input type="file" class="upload" id="J_canvas_bg" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp,image/webp"/>',
            '</div>',
            '</div>',
            '<div class="sectionBar">',
                ...
                yesBtn: {
                    ...
                    click: function (e) {
                        ...
                        if (typeof self.wordImageServer !== 'undefined' && self.wordImageServer) {
                            var i = 0;
                            var arr = [];
                            var replacedStr = str.replace(/file:\/\/+(localhost)?(\S+\.(png|jpg|jpeg|gif|bmp|webp))/ig, function (value) {
                                arr[i] = value;
                                var replaced = "##" + i + "##";
                                i++;
                                return replaced;
