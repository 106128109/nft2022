...
class Study extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $list = $this->model
                    ->with(['studycategory'])
                    ->where(['study.is_del'=>0,'studycategory.is_del'=>0])
                    ->where($where)
                    ->order($sort, $order)
                    ->paginate($limit);

            foreach ($list as $row) {
                $row->visible(['id','image','title','desc','order','content','is_show','create_time']);
                $row->visible(['studycategory']);
				$row->getRelation('studycategory')->visible(['name']);
            }
